    ...
    public class SceneGroupManager
    {
        ...
        internal async Task UnloadScenes()
        {
            ...
            for (var i = 0; i < SceneManager.sceneCount; i++)
            {
                ...
                var sceneData = _activeSceneGroup.Scenes.FirstOrDefault(sceneData => sceneData.Name == scene.name);
                if (!scene.isLoaded || scene.name == activeSceneName || scene.name == "Bootstrapper" || sceneData.SceneType == SceneType.Persistent)
                    continue;
            ...
            foreach (var sceneName in scenesToUnload)
            {
                ...
                if (operation != null) operationGroup.Operations.Add(operation);
